...
@Component
public class BasketOddValidator implements MatchOddValidator {
	...
	public boolean isValid(List<MatchOddDto> odds) {
		if (odds.size() != Sport.BASKET.getNumberOfValidOdds()) {
			return false;
		}
	...
	private boolean checkSpecifiers(List<MatchOddDto> odds) {
		return Arrays.asList(Specifier.values()).stream().filter(s -> s.isBasketValid()).map(s -> countOddType(s.getCode(), odds) == 1).allMatch(Boolean::booleanValue);
	}
